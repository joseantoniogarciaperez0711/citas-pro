...
@auth
    @if (Auth::user()->business_logo_path)
        <img src="{{ Auth::user()->business_logo_url }}?v={{ optional(Auth::user()->updated_at)->timestamp }}" alt="Logo"
            {{ $attributes->merge(['class' => $class . ' rounded-lg object-cover']) }}>
    @else
        {{-- Logo por defecto de Jetstream (deja tu SVG actual) --}}
    ...
    @endif
@else
    {{-- Visitante (login/register): muestra el SVG por defecto --}}
